...
import { getDocumentUrl, resolveDocumentUrl } from '../../utils/uploadDocument';

function resolveStoredUrl(doc) {
  if (!doc || typeof doc !== 'object') return null;
  if (doc.viewUrl) return resolveDocumentUrl(doc.viewUrl);
...
function DocumentPreviewCard({ label, document: doc }) {
  ...
  const cachedUrl = resolveStoredUrl(doc);

  const { data: fetchedUrl, isLoading, isError } = useQuery({
    queryKey: ['doc-url', docId],
    queryFn: () => getDocumentUrl(docId),
    enabled: Boolean(docId) && !cachedUrl,
  });

  const url = cachedUrl || fetchedUrl;
  const loading = !cachedUrl && isLoading;
  ...
  return (
    <Card variant="outlined" sx={{ height: '100%' }}>
      ...
      >
        ...
        {!loading && !isImage && !isPdf && url && (
          <Typography variant="body2" color="text.secondary">File ready</Typography>
        )}
        {isError && !cachedUrl && <Typography variant="caption" color="error">Failed to load</Typography>}
      </Box>
